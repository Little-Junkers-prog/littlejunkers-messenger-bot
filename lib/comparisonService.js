...
function observationMatchesMarket(row, zip) { if (row.market_zip) return row.market_zip === zip; return true; }
...
export async function getPublicComparison({ zip, sizeClass, tierKey = "3day" }) {
  ...
  return { zip: zip || null, sizeClassYards: requestedClass, freshnessDays: PUBLIC_FRESHNESS_DAYS, littleJunkers, competitors, providers: [littleJunkers, ...competitors] };
}
