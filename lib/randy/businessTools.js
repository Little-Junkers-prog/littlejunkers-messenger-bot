...
function buildWindows(tier, blocked, today, windowEnd, price = null) {
...
export async function getAvailabilityContext({ sizeYards, zip } = {}) {
  ...
  const tiers = pricingConfig.pricing.map((tier) => {
    ...
    if (zip) {
      try {
        price = resolveTierPrice(pricingConfig, {
          tierKey: tier.tierKey,
          sizeYards: normalizedSizeYards,
          zip,
        });
    ...
    return {
      tier,
      price,
      windows: buildWindows(tier, blocked, today, windowEnd, price),
    };
  });
...
export async function createRandySession(payload = {}) {
  ...
  if (error) {
    // The table may not have every rollout column yet. Retry without optional columns before breaking chat.
    const fallbackRow = { ...row };
    delete fallbackRow.name;
    const fallback = await supabase.from("randy_sessions").insert(fallbackRow).select("id").single();
    if (fallback.error) {
      console.warn("[randy] randy_sessions insert skipped:", fallback.error.message);
      return null;
    }
...
export function buildPrefilledBookingUrl({ sessionId, zip, sizeYards, projectType } = {}) {
  const rawBase = process.env.NEXT_PUBLIC_BOOKING_URL || process.env.BOOKING_URL || "https://book.littlejunkersllc.com/rent-a-dumpster";
  const base = rawBase.replace(/\/rent-a-dumpster\/?$/, "/randy-booking");
