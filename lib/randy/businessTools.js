...
export async function getAvailabilityContext({ sizeYards, zip } = {}) {
  ...
  for (const tier of pricingConfig.pricing) {
    ...
    const windows = await getTierAvailability({
      sizeYards: normalizedSizeYards,
      tier,
      today,
      windowEnd,
      maxWindows: MAX_WINDOWS_PER_TIER,
      price,
    });
...
export function buildPrefilledBookingUrl({ sessionId, zip, sizeYards, projectType } = {}) {
  // zip is accepted in the signature for internal use but intentionally not appended to the URL.
  // Sprint 3.5: ZIP gate removed from all funnel entry points. ZIP is collected later in the booking flow.
