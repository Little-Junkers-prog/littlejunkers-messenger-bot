...
// Extract an explicitly requested dumpster size before project-type inference.
// Plural "yards" is ambiguous outside a rental context (for example, a distance
// from a property line), so require nearby dumpster/rental language for it.
export function extractExplicitSizeYards(text = "") {
  const raw = String(text || "");
  const sizePattern = /\b(11|16|21)\s*-?\s*(yds?|yards?|yarder|cys?|cy)\b/gi;
  const rentalContextPattern = /\b(dumpster|rental|rent|book|booking|container|roll[\s-]?off|debris|junk|cleanout|remodel|construction|deliver|drop[- ]?off|need|want)\b/i;
  const locationPattern = /\b(from|to|between|away|along|near)\b.{0,40}\b(property\s+line|street|fence|sidewalk|driveway|curb|house)\b/i;

  let match;
  while ((match = sizePattern.exec(raw))) {
    const unit = match[2].toLowerCase();
    const contextStart = Math.max(0, match.index - 48);
    const contextEnd = Math.min(raw.length, match.index + match[0].length + 64);
    const context = raw.slice(contextStart, contextEnd);
    const pluralUnit = /^(yds|yards|cys)$/i.test(unit);

    if (pluralUnit && (!rentalContextPattern.test(context) || locationPattern.test(context))) {
      continue;
    }
