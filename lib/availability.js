// lib/availability.js
// Sprint 2A: queries public.units and public.rentals (Supabase source of truth)
...
export async function getAvailabilitySnapshot({
  ...
  // Units already in the field are not available regardless of rental records
  for (const unit of candidateUnits) {
    if (unit.status === "deployed") {
      availableUnitIds.delete(unit.id);
      blockedUnits.push(buildUnitResult(unit, "deployed"));
    }
  }

  // Remove units tied to confirmed/active rentals in this window
  const assignedBlocks = [];
  const unassignedBlockingCount = blockingRentals.filter((r) => !r.unit_id).length;
  ...
  return {
    ...
    totals: {
      ...
      unassignedBlockingRentals: unassignedBlockingCount,
      tightWindow,
    },
    availableUnits,
    blockedUnits,
