...
const NON_RENTABLE_UNIT_STATUSES = new Set(["maintenance", "retired", "unavailable", "out_of_service"]);
...
function normalizeUnit(row) {
  const sizeYards = normalizeSizeYards(pick(row, ["size_yards", "sizeYards", "yards", "size", "size_code", "sizeCode"]));
  const status = asString(pick(row, ["status", "unit_status"], "ready")).toLowerCase();
  return {
    id: pick(row, ["id", "unit_id", "dumpster_unit_id"]),
    name: pick(row, ["name", "unit_number", "label"], ""),
    sizeYards,
...
function normalizeCommitment(row) {
  const sizeYards = normalizeSizeYards(pick(row, ["size_yards", "sizeYards", "yards", "size", "size_code", "sizeCode"]));
  const start = parseDateOnly(pick(row, ["start_date", "dropoff_date", "requested_start_date", "requested_start_at", "start_at", "start"]));
  const end = parseDateOnly(pick(row, ["return_date", "scheduled_return", "end_date", "requested_end_date", "requested_end_at", "end_at", "end"]));
...
export async function getFleetCapacity({ sizeYards } = {}) {
  ...
  if (normalizedSizeYards) query = query.eq("size_yards", normalizedSizeYards);
...
export async function getActiveCommitments({ sizeYards, startDate, endDate, now = new Date() } = {}) {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .lt("start_date", toDateStr(end))
    .gt("return_date", toDateStr(start));
...
export async function evaluateWindow({ sizeYards, startDate, endDate, now = new Date(), audit = false, source = "availabilityService" } = {}) {
  ...
  const capacity = bySize[sizeCode]?.total || 0;
...
async function buildBlockedDates({ sizeYards, today, windowEnd, now }) {
  const normalizedSizeYards = normalizeSizeYards(sizeYards);
  const blocked = new Set();
  const cur = new Date(today);
  while (cur <= windowEnd) {
    const nextDay = addDays(cur, 1);
    const result = await evaluateWindow({ sizeYards: normalizedSizeYards, startDate: cur, endDate: nextDay, now });
...
export async function getTierAvailability({ sizeYards, tier, today, windowEnd, maxWindows = MAX_WINDOWS_PER_TIER, price = null } = {}) {
  const now = new Date();
  const blocked = await buildBlockedDates({ sizeYards, today, windowEnd, now });
...
export async function getAvailabilityCalendar({ sizeYards, maxWindows = MAX_WINDOWS_PER_TIER } = {}) {
  ...
  const blocked = await buildBlockedDates({ sizeYards: normalizedSizeYards, today, windowEnd, now: new Date() });
  ...
  const snapshot = await evaluateWindow({ sizeYards: normalizedSizeYards, startDate: addDays(today, 1), endDate: addDays(today, 2) });

  return {
    sizeYards: normalizedSizeYards,
    sizeCode: sizeYardsToCode(normalizedSizeYards),
    available,
...
export async function getCsrAvailabilitySummary() {
  ...
  for (const sizeCode of Object.keys(counts.bySize)) {
    ...
    const evalNow = await evaluateWindow({ sizeYards: yards, startDate: today, endDate: tomorrow });
...
export async function createBookingHold({ sizeYards, startDate, endDate, customerId = null, holdType = "online_checkout", source = "funnel", metadata = {} } = {}) {
  assertServerOnly();
  const supabase = getSupabaseAdmin();
  const settings = await getAvailabilitySettings(supabase);
  ...
  const holdMinutes = getHoldMinutes(holdType, settings);
  ...
  const attempts = [
    base,
    { ...base, dropoff_date: start, scheduled_return: end },
    { hold_type: holdType, status: "active", size_yards: normalizedSizeYards, dropoff_date: start, scheduled_return: end, expires_at: expiresAt, customer_id: customerId, source, metadata },
  ];
