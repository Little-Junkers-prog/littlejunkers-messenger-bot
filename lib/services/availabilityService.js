...
export async function getFleetCapacity({ sizeYards } = {}) {
  ...
  const normalizedSizeYards = normalizeSizeYards(sizeYards);
  const requestedSizes = normalizedSizeYards ? [normalizedSizeYards] : [11, 16, 21];

  // RPC capacity is lifecycle-only for future booking windows. Current readiness
  // and truck status remain intentionally outside this calculation (PL-045).
  const capacityResults = await Promise.all(
    requestedSizes.map((yards) => supabase.rpc("get_fleet_capacity", { p_size_yards: yards })),
  );
  for (const result of capacityResults) {
    if (result.error) throw result.error;
  }

  let unitQuery = supabase.from("dumpster_units").select("*");
  if (normalizedSizeYards) unitQuery = unitQuery.eq("size_code", sizeYardsToCode(normalizedSizeYards));
  const { data: unitRows, error: unitError } = await unitQuery;
  if (unitError) throw unitError;

  const units = (unitRows || []).map(normalizeUnit).filter((unit) => unit.sizeYards);
  const bySize = capacityResults.reduce((acc, result, index) => {
    const yards = requestedSizes[index];
    const sizeCode = sizeYardsToCode(yards);
    const total = Number(result.data || 0);
    acc[sizeCode] = {
      sizeYards: yards,
      total: Number.isFinite(total) ? total : 0,
      units: units.filter((unit) => unit.sizeCode === sizeCode),
    };
    return acc;
  }, {});
  ...
  return {
    ...
    rentableUnits: units,
    capacityUnits: units,
    physicallyReadyUnits: units.filter((unit) => unit.status === "ready"),
    bySize,
  };
}
