...
export async function recordStripeRevenueLedgerEntry(payment, rental, balanceTransactionFee = null) {
  ...
  if (existingRevenue) return { revenueTransaction: existingRevenue, revenueAlreadyRecorded: true, feeTransaction: null, feeAlreadyRecorded: false };
  const revenue = await insertTransactionWithAllocation(supabase, {
  ...
  return { revenueTransaction: revenue.transaction, revenueAllocation: revenue.allocation, revenueAlreadyRecorded: false, feeTransaction: feeResult?.transaction || null, feeAllocation: feeResult?.allocation || null, feeAlreadyRecorded: Boolean(feeResult?.alreadyRecorded) };
}