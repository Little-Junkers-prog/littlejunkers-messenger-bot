...
export async function getPricingConfig() {
  ...
  const [pricingRes, serviceAreasRes, zipCodesRes, sizesRes, feesRes] = await Promise.all([
    ...
    supabase.from("service_areas").select("*").eq("active", true),
    supabase.from("zip_codes").select("*").eq("active", true),
    supabase.from("dumpster_sizes").select("*"),
    supabase.from("fees").select("*").eq("active", true),
  ]);
  ...
  failIfSupabaseError("fees", feesRes, { optional: true });
